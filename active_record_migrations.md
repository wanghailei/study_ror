...
> Why it's an index? why not a constraint?
>
> ```ruby
> # Database-level constraint to prevent duplicate links 
> add_index :formulations, [:product_id, :formula_id], unique: true, name: "index_unique_formulations"
> ```

Gemini: It is **both**. In relational databases (like SQLite), ==a **Unique Constraint** is physically implemented *using* a **Unique Index**.== They are two sides of the same coin:

**The Constraint is the "Law" (Logic):** It defines the rule: "No duplicates allowed." This satisfies your requirement for Order. **The Index is the "Enforcer" (Mechanism):** To enforce the law, the database must check every new entry against existing ones. ==Therefore, when you write `add_index ..., unique: true` in Rails, you are instructing the database to build the **Index** (the structure) required to efficiently enforce the **Constraint** (the rule).== Technically, you could write raw SQL to add a constraint, but the database would still generate an index behind the scenes to make it work. Rails simply exposes the mechanism directly.
