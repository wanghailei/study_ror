...
It feels like a traditional page navigation (URL changes, back/forward work, HTML comes from the server), but behaves as smoothy as an SPA under the hood. Therefore, I call Turbo Drive "full-page refresh 2.0".

All browser features still work - back button, history, SEO, etc. Pages look the same — just faster and smoother. You don’t have to change your routing or layout. Works with normal Rails controllers and ERB views.

#### What makes it “2.0”?

| **1.0 Classic Full Refresh**       | **2.0 Turbo Drive**                    |
| ---------------------------------- | -------------------------------------- |
| Browser reloads whole page         | Only swaps `<body>` and `<title>`      |
| JS and CSS reload every time       | JS, CSS, and state persist             |
| Flash of white (layout re-renders) | Seamless transition, no layout flicker |
| Slow navigation feel               | Instant page changes                   |
...
Application visits are initiated by clicking a Turbo Drive-enabled link, or programmatically by calling [`Turbo.visit(location)`](https://turbo.hotwired.dev/reference/drive#turbodrivevisit).
...
The default visit action is *advance*. During an advance visit, Turbo Drives pushes a new entry onto the browser’s history stack using [`history.pushState`](https://developer.mozilla.org/en-US/docs/Web/API/History/pushState).

You may wish to visit a location without pushing a new history entry onto the stack. The *replace* visit action uses [`history.replaceState`](https://developer.mozilla.org/en-US/docs/Web/API/History/replaceState) to discard the topmost history entry and replace it with the new location.
...
Listen for the `turbo:before-visit` event to be notified when a visit is about to start, and use `event.detail.url` to check the visit’s location. Then cancel the visit by calling `event.preventDefault()`.
...
## [﹟](https://turbo.hotwired.dev/handbook/drive#pausing-requests)Pausing Requests
...
In browsers that support the [View Transition API](https://developer.mozilla.org/en-US/docs/Web/API/View_Transitions_API) Turbo can trigger view transitions when navigating between pages.
...
## [﹟](https://turbo.hotwired.dev/handbook/drive#removing-assets-when-they-change)Removing Assets When They Change
...
## [﹟](https://turbo.hotwired.dev/handbook/drive#setting-a-root-location)Setting a Root Location
...
## [﹟](https://turbo.hotwired.dev/handbook/drive#form-submissions)Form Submissions
...
Throughout a submission, Turbo Drive will dispatch a series of [events](https://turbo.hotwired.dev/reference/events) that target the `<form>` element and [bubble up](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Building_blocks/Events#event_bubbling) through the document:
...
During a submission, Turbo Drive will set the “submitter” element’s [disabled](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/disabled) attribute when the submission begins, then remove the attribute after the submission ends. When submitting a `<form>` element, browsers will treat the `<input type="submit">` or `<button>` element that initiated the submission as the [submitter](https://developer.mozilla.org/en-US/docs/Web/API/SubmitEvent/submitter). To submit a `<form>` element programmatically, invoke the [HTMLFormElement.requestSubmit()](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/requestSubmit) method and pass an `<input type="submit">` or `<button>` element as an optional parameter.

If there are other changes you’d like to make during a `<form>` submission (for example, disabling *all* [fields within a submitted ``](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/elements)), you can declare your own event listeners:
...
## [﹟](https://turbo.hotwired.dev/handbook/drive#streaming-after-a-form-submission)Streaming After a Form Submission

Servers may also respond to form submissions with a [Turbo Streams](https://turbo.hotwired.dev/handbook/streams) message by sending the header `Content-Type: text/vnd.turbo-stream.html` followed by one or more `<turbo-stream>` elements in the response body. This lets you update multiple parts of the page without navigating.

## [﹟](https://turbo.hotwired.dev/handbook/drive#prefetching-links-on-hover)Prefetching Links on Hover
...
Preload links into Turbo Drive’s cache using the [data-turbo-preload](https://turbo.hotwired.dev/reference/attributes#data-attributes) boolean attribute.
...
Not every `<a>` element can be preloaded. The `[data-turbo-preload]` attribute won’t have any effect on links that:
...
It also dovetails nicely with pages that leverage [Eager-Loading Frames](https://turbo.hotwired.dev/reference/frames#eager-loaded-frame) or [Lazy-Loading Frames](https://turbo.hotwired.dev/reference/frames#lazy-loaded-frame). As you can preload the structure of the page and show the user a meaningful loading state while the interesting content loads.

## [﹟](https://turbo.hotwired.dev/handbook/drive#ignored-paths)Ignored Paths
...
Note that preloaded `<a>` elements will dispatch [turbo:before-fetch-request](https://turbo.hotwired.dev/reference/events) and [turbo:before-fetch-response](https://turbo.hotwired.dev/reference/events) events. To distinguish a preloading `turbo:before-fetch-request` initiated event from an event initiated by another mechanism, check whether the request’s `X-Sec-Purpose` header (read from the `event.detail.fetchOptions.headers["X-Sec-Purpose"]` property) is set to `"prefetch"`:
