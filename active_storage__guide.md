...
| `active_storage_attachments`     | A polymorphic join table that [connects your models to blobs](https://guides.rubyonrails.org/active_storage_overview.html#attaching-files-to-records). If your model's class name changes, you will need to run a migration on this table to update the underlying `record_type` to your model's new class name. |
| `active_storage_variant_records` | If [variant tracking](https://guides.rubyonrails.org/active_storage_overview.html#attaching-files-to-records) is enabled, stores records for each variant that has been generated. |

### Declare Active Storage services in `config/storage.yml`.

For each service your application uses, provide a name and the requisite configuration. The example below declares three services named `local`, `test`, and `amazon`:
...
### [2.1. Disk Service](https://guides.rubyonrails.org/active_storage_overview.html#disk-service)
...
### [2.2. S3 Service (Amazon S3 and S3-compatible APIs)](https://guides.rubyonrails.org/active_storage_overview.html#s3-service-amazon-s3-and-s3-compatible-apis)
...
The core features of Active Storage require the following permissions: `s3:ListBucket`, `s3:PutObject`, `s3:GetObject`, and `s3:DeleteObject`. [Public access](https://guides.rubyonrails.org/active_storage_overview.html#public-access) additionally requires `s3:PutObjectAcl`. If you have additional upload options configured such as setting ACLs then additional permissions may be required.
...
### [2.3. Google Cloud Storage Service](https://guides.rubyonrails.org/active_storage_overview.html#google-cloud-storage-service)
...
### [2.4. Mirror Service](https://guides.rubyonrails.org/active_storage_overview.html#mirror-service)
...
### [2.5. Public access](https://guides.rubyonrails.org/active_storage_overview.html#public-access)
...
## [3. Attaching Files to Records](https://guides.rubyonrails.org/active_storage_overview.html#attaching-files-to-records)

### [3.1. `has_one_attached`](https://guides.rubyonrails.org/active_storage_overview.html#attaching-files-to-records-has-one-attached)

The [`has_one_attached`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/Model.html#method-i-has_one_attached) macro sets up a one-to-one mapping between records and files. Each record can have one file attached to it.
...
Call [`avatar.attach`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/One.html#method-i-attach) to attach an avatar to an existing user:
...
Call [`avatar.attached?`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/One.html#method-i-attached-3F) to determine whether a particular user has an avatar:
...
### [3.2. `has_many_attached`](https://guides.rubyonrails.org/active_storage_overview.html#attaching-files-to-records-has-many-attached)

The [`has_many_attached`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/Model.html#method-i-has_many_attached) macro sets up a one-to-many relationship between records and files. Each record can have many files attached to it.
...
Call [`images.attach`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/Many.html#method-i-attach) to add new images to an existing message:
...
Call [`images.attached?`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/Many.html#method-i-attached-3F) to determine whether a particular message has any images:
...
### [3.3. Attaching File/IO Objects](https://guides.rubyonrails.org/active_storage_overview.html#attaching-file-io-objects)
...
### [3.4. Replacing vs Adding Attachments](https://guides.rubyonrails.org/active_storage_overview.html#replacing-vs-adding-attachments)

By default in Rails, attaching files to a `has_many_attached` association will replace any existing attachments.

To keep existing attachments, you can use hidden form fields with the [`signed_id`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html#method-i-signed_id) of each attached file:
...
### [3.5. Form Validation](https://guides.rubyonrails.org/active_storage_overview.html#form-validation)

Attachments aren't sent to the storage service until a successful `save` on the associated record. This means that if a form submission fails validation, any new attachment(s) will be lost and must be uploaded again. Since [direct uploads](https://guides.rubyonrails.org/active_storage_overview.html#direct-uploads) are stored before the form is submitted, they can be used to retain uploads when validation fails:
...
## [4. Querying](https://guides.rubyonrails.org/active_storage_overview.html#querying)
...
### [4.1. `has_one_attached`](https://guides.rubyonrails.org/active_storage_overview.html#querying-has-one-attached)

[`has_one_attached`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/Model.html#method-i-has_one_attached) creates a `has_one` association named `"<name>_attachment"` and a `has_one :through` association named `"<name>_blob"`. To select every user where the avatar is a PNG, run the following:
...
The query will filter on the [**`ActiveStorage::Blob`**](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html), not the [attachment record](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attachment.html) because these are plain SQL joins. You can combine the blob predicates above with any other scope conditions, just as you would with any other Active Record query.

## [5. Removing Files](https://guides.rubyonrails.org/active_storage_overview.html#removing-files)

To remove an attachment from a model, call [`purge`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/One.html#method-i-purge) on the attachment. If your application is set up to use Active Job, removal can be done in the background instead by calling [`purge_later`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attached/One.html#method-i-purge_later). Purging deletes the blob and the file from the storage service.
...
## [6. Serving Files](https://guides.rubyonrails.org/active_storage_overview.html#serving-files)
...
### [6.1. Redirect Mode](https://guides.rubyonrails.org/active_storage_overview.html#redirect-mode)

To generate a permanent URL for a blob, you can pass the attachment or the blob to the [`url_for`](https://api.rubyonrails.org/v8.1.1/classes/ActionView/RoutingUrlFor.html#method-i-url_for) view helper. This generates a URL with the blob's [`signed_id`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html#method-i-signed_id) that is routed to the blob's [`RedirectController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blobs/RedirectController.html)
...
### [6.2. Proxy Mode](https://guides.rubyonrails.org/active_storage_overview.html#proxy-mode)
...
#### [6.2.1. Putting a CDN in Front of Active Storage](https://guides.rubyonrails.org/active_storage_overview.html#putting-a-cdn-in-front-of-active-storage)
...
### [6.3. Authenticated Controllers](https://guides.rubyonrails.org/active_storage_overview.html#authenticated-controllers)

All Active Storage controllers are publicly accessible by default. The generated URLs use a plain [`signed_id`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html#method-i-signed_id), making them hard to guess but permanent. Anyone that knows the blob URL will be able to access it, even if a `before_action` in your `ApplicationController` would otherwise require a login. If your files require a higher level of protection, you can implement your own authenticated controllers, based on the [`ActiveStorage::Blobs::RedirectController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blobs/RedirectController.html),[`ActiveStorage::Blobs::ProxyController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blobs/ProxyController.html),[`ActiveStorage::Representations::RedirectController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Representations/RedirectController.html) and[`ActiveStorage::Representations::ProxyController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Representations/ProxyController.html)
...
## [7. Downloading Files](https://guides.rubyonrails.org/active_storage_overview.html#downloading-files)

Sometimes you need to process a blob after it’s uploaded—for example, to convert it to a different format. Use the attachment's [`download`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html#method-i-download) method to read a blob’s binary data into memory:
...
You might want to download a blob to a file on disk so an external program (e.g. a virus scanner or media transcoder) can operate on it. Use the attachment's [`open`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob.html#method-i-open) method to download a blob to a tempfile on disk:
...
## [8. Analyzing Files](https://guides.rubyonrails.org/active_storage_overview.html#analyzing-files)

Active Storage analyzes files once they've been uploaded by queuing a job in Active Job. Analyzed files will store additional information in the metadata hash, including `analyzed: true`. You can check whether a blob has been analyzed by calling [`analyzed?`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob/Analyzable.html#method-i-analyzed-3F) on it.
...
## [9. Displaying Images, Videos, and PDFs](https://guides.rubyonrails.org/active_storage_overview.html#displaying-images-videos-and-pdfs)

Active Storage supports representing a variety of files. You can call [`representation`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob/Representable.html#method-i-representation) on an attachment to display an image variant, or a preview of a video or PDF. Before calling `representation`, check if the attachment can be represented by calling [`representable?`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob/Representable.html#method-i-representable-3F). Some file formats can't be previewed by Active Storage out of the box (e.g. Word documents); if `representable?` returns false you may want to [link to](https://guides.rubyonrails.org/active_storage_overview.html#serving-files) the file instead.
...
### [9.1. Lazy vs Immediate Loading](https://guides.rubyonrails.org/active_storage_overview.html#lazy-vs-immediate-loading)
...
Will generate an `<img>` tag with the `src` pointing to the[`ActiveStorage::Representations::RedirectController`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Representations/RedirectController.html). The browser will make a request to that controller, which will perform the following:
...
2. Return a `302` redirect to the file either to
	...
	- or `ActiveStorage::Blobs::ProxyController` which will return the file contents if [proxy mode](https://guides.rubyonrails.org/active_storage_overview.html#proxy-mode) is enabled.

Loading the file lazily allows features like [single use URLs](https://guides.rubyonrails.org/active_storage_overview.html#public-access) to work without slowing down your initial page loads.
...
If you're rendering lots of images on a page, the above example could result in N+1 queries loading all the variant records. To avoid these N+1 queries, use the named scopes on [`ActiveStorage::Attachment`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Attachment.html).
...
### [9.2. Transforming Images](https://guides.rubyonrails.org/active_storage_overview.html#transforming-images)

Transforming images allows you to display the image at your choice of dimensions. To create a variation of an image, call [`variant`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob/Representable.html#method-i-variant) on the attachment. You can pass any transformation supported by the variant processor to the method. When the browser hits the variant URL, Active Storage will lazily transform the original blob into the specified format and redirect to its new service location.
...
### [9.3. Previewing Files](https://guides.rubyonrails.org/active_storage_overview.html#previewing-files)

Some non-image files can be previewed: that is, they can be presented as images. For example, a video file can be previewed by extracting its first frame. Out of the box, Active Storage supports previewing videos and PDF documents. To create a link to a lazily-generated preview, use the attachment's [`preview`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Blob/Representable.html#method-i-preview) method:
...
To add support for another format, add your own previewer. See the [`ActiveStorage::Preview`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Preview.html)documentation for more information.

## [10. Direct Uploads](https://guides.rubyonrails.org/active_storage_overview.html#direct-uploads)

Active Storage, with its included JavaScript library, supports uploading directly from the client to the cloud.

### [10.1. Usage](https://guides.rubyonrails.org/active_storage_overview.html#usage)
...
### [10.2. Cross-Origin Resource Sharing (CORS) Configuration](https://guides.rubyonrails.org/active_storage_overview.html#cross-origin-resource-sharing-cors-configuration)
...
#### [10.2.1. Example: S3 CORS Configuration](https://guides.rubyonrails.org/active_storage_overview.html#example-s3-cors-configuration)
...
#### [10.2.2. Example: Google Cloud Storage CORS Configuration](https://guides.rubyonrails.org/active_storage_overview.html#example-google-cloud-storage-cors-configuration)
...
### [10.3. Direct Upload JavaScript Events](https://guides.rubyonrails.org/active_storage_overview.html#direct-upload-javascript-events)
...
### [10.4. Example](https://guides.rubyonrails.org/active_storage_overview.html#example)
...
### [10.5. Custom drag and drop solutions](https://guides.rubyonrails.org/active_storage_overview.html#custom-drag-and-drop-solutions)
...
### [10.6. Track the progress of the file upload](https://guides.rubyonrails.org/active_storage_overview.html#track-the-progress-of-the-file-upload)
...
### [10.7. Integrating with Libraries or Frameworks](https://guides.rubyonrails.org/active_storage_overview.html#integrating-with-libraries-or-frameworks)
...
Using [Direct Uploads](https://guides.rubyonrails.org/active_storage_overview.html#direct-uploads) can sometimes result in a file that uploads, but never attaches to a record. Consider [purging unattached uploads](https://guides.rubyonrails.org/active_storage_overview.html#purging-unattached-uploads).

## [11. Testing](https://guides.rubyonrails.org/active_storage_overview.html#testing)

Use [`file_fixture_upload`](https://api.rubyonrails.org/v8.1.1/classes/ActionDispatch/TestProcess/FixtureFile.html#method-i-file_fixture_upload) to test uploading a file in an integration or controller test. Rails handles files like any other parameter.
...
### [11.1. Discarding Files Created During Tests](https://guides.rubyonrails.org/active_storage_overview.html#discarding-files-created-during-tests)

#### [11.1.1. System Tests](https://guides.rubyonrails.org/active_storage_overview.html#system-tests)
...
#### [11.1.2. Integration Tests](https://guides.rubyonrails.org/active_storage_overview.html#integration-tests)
...
### [11.2. Adding Attachments to Fixtures](https://guides.rubyonrails.org/active_storage_overview.html#adding-attachments-to-fixtures)
...
Then put a file in your fixtures directory (the default path is `test/fixtures/files`) with the corresponding filename. See the [`ActiveStorage::FixtureSet`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/FixtureSet.html) docs for more information.
...
#### [11.2.1. Cleaning up Fixtures](https://guides.rubyonrails.org/active_storage_overview.html#cleaning-up-fixtures)

While files uploaded in tests are cleaned up [at the end of each test](https://guides.rubyonrails.org/active_storage_overview.html#discarding-files-created-during-tests), you only need to clean up fixture files once: when all your tests complete.
...
### [11.3. Configuring services](https://guides.rubyonrails.org/active_storage_overview.html#configuring-services)
...
## [12. Implementing Support for Other Cloud Services](https://guides.rubyonrails.org/active_storage_overview.html#implementing-support-for-other-cloud-services)

If you need to support a cloud service other than these, you will need to implement the Service. Each service extends [`ActiveStorage::Service`](https://api.rubyonrails.org/v8.1.1/classes/ActiveStorage/Service.html) by implementing the methods necessary to upload and download files to the cloud.

## [13. Purging Unattached Uploads](https://guides.rubyonrails.org/active_storage_overview.html#purging-unattached-uploads)

There are cases where a file is uploaded but never attached to a record. This can happen when using [Direct Uploads](https://guides.rubyonrails.org/active_storage_overview.html#direct-uploads). You can query for unattached records using the [unattached scope](https://github.com/rails/rails/blob/8ef5bd9ced351162b673904a0b77c7034ca2bc20/activestorage/app/models/active_storage/blob.rb#L49). Below is an example using a [custom rake task](https://guides.rubyonrails.org/command_line.html#custom-rake-tasks).
