...
Active Record will perform queries on the database for you and is compatible with most database systems, including MySQL, MariaDB, PostgreSQL, and SQLite. Regardless of which database system you're using, the Active Record method format will always be the same. All models use `id` as the primary key, unless specified otherwise.
...
- Run `after_find` then `after_initialize` callbacks, if any.
...
The `id_value` method can be used to fetch the value of the `:id` column for a record, for use in finder methods such as `find_by` and `where`. See example below:
...
If an order is present in the receiver the behavior depends on the flag `config.active_record.error_on_ignored_order`. If true, `ArgumentError` is raised, otherwise the order is ignored and a warning issued, which is the default. This can be overridden with the option `:error_on_ignore`, explained below.
...
class Book
    # Returns reviews if there are at least 5, else consider this as non-reviewed book
...
==Unlike `select`, `pluck` directly converts a database result into a Ruby `Array`, without constructing `ActiveRecord` objects. This can mean better performance for a large or frequently-run query. However, any model method overrides will not be available.== 
...
##### What pluck really does?

==`pluck` tells Active Record to **directly fetch columns** from the database — skipping model instantiation. It’s the fastest way to get raw data arrays.==

==`pluck` is Active Record’s shortcut for a direct SQL SELECT query== that:

- Retrieves *only the columns you specify*,
- Returns them as plain Ruby arrays,
- Skips model instantiation entirely.

==Internally, pluck is implemented in pure SQL.==
...
That means it uses *ActiveRecord’s database adapter* directly — no model logic, no callbacks, no type casting (except by the DB adapter).

##### It’s much faster than `.map` or `.select`.

==For large datasets, `pluck` can be **10–100× faster** than iterating over model instances.==
...
- Loads *every user record* into memory.
- Instantiates *User objects*.
- Calls the email method on each.
- Memory-hungry and slower.
...
- Fetches just the email column.
- No Ruby objects created.
- Returns raw Ruby values directly.
- Extremely efficient.

##### It works directly on relations (including joins, scopes, etc.)

You can chain it with:

```ruby
Order.joins(:customer).where(status: :paid).pluck("customers.name", "orders.total")
```

→ runs a single optimized SQL query

→ returns a lightweight array of results.

##### It respects table aliases and avoids ambiguity.

When you use hash syntax:

```ruby
Order.joins(:customer).pluck(orders: [:id, :total], customers: [:name])
```

Rails automatically builds:

```ruby
SELECT "orders"."id", "orders"."total", "customers"."name"
```

That prevents ambiguous column errors (like id appearing in multiple tables).

##### `pluck` use cases

| **Use Case**          | **Why** pluck **is ideal**                           |
| --------------------- | ---------------------------------------------------- |
| Dashboards, analytics | Fetch just the metrics you need                      |
| Dropdown lists        | e.g., User.pluck(:name, :id) for <select>            |
| Exporting data        | Fast, minimal memory usage                           |
| Joins for reports     | Combine multiple tables, get only fields of interest |

##### When not to use `pluck`?

- When you need full ActiveRecord model behaviour (valid?, callbacks, associations).
- When you need to *modify or save* records.
- When you want lazily chainable scopes (pluck executes immediately).
